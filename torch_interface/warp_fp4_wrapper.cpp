...
torch::Tensor csrspmm_warp_fp4_forward(
    Tensor crow,
    Tensor col,
    Tensor values,
    Tensor B
    ...
    TORCH_CHECK((N % 4) == 0, "csrspmm_warp_fp4 requires N divisible by 4");

    // alignment checks (your kernel assumes float4-aligned B and C)
    auto b_ptr = reinterpret_cast<std::uintptr_t>(B.data_ptr<float>());
    TORCH_CHECK((b_ptr % 16) == 0, "B must be 16-byte aligned for csrspmm_warp_fp4");

    Tensor C = torch::zeros({M, N}, B.options());
    auto c_ptr = reinterpret_cast<std::uintptr_t>(C.data_ptr<float>());
    TORCH_CHECK((c_ptr % 16) == 0, "C must be 16-byte aligned for csrspmm_warp_fp4");
    ...
    csrspmm::launch_warp_per_row_fp4(dA, dB, dC, 1.0f, 0.0f);
