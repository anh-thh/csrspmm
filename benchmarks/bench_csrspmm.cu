...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      ((arg == "-M")    && i + 1 < argc) M        = std::atoi(argv[++i]);
            ...
            std::cout <<
                ...
                "  -b <float>    beta scalar\n"
                "  -algo <str>   Naive | WarpPerRow | WarpPerRowFp4 | "
                                  "WarpPerRowSmem | WarpPerRowSmemFp4 | NaiveShared\n"
                "Example:\n"
                "  ./bench_csr_spmm -M 4096 -N 4096 -K 256 -s 0.999 -algo NaiveShared\n";
            return 0;
        }
    ...
    elapsed_time /= 1000.f; // seconds

    double flops = 2.0 * dA_CSR.nnz * K;  // 2 ops per nonzero × K columns
    printf(
